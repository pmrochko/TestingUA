        ...
        <c:if test="${sessionScope.testStatus.name().equals('TIMEOUT')}">
            <div class="timeoutInfo">
                ...
                <h5 class="timeoutInfo-title">
                    ...
                    <fmt:message key="student.resultTestPage.timeoutBOX.title"/>
                </h5>

                <hr style="margin: 0;">

                <p class="timeoutInfo-description">
                    <fmt:message key="student.resultTestPage.timeoutBOX.description"/>
                </p>
            </div>
        </c:if>

        <div class="result-window bg-dark">
            <h3 style="margin: 0 0 20px 20px;">
                <fmt:message key="student.resultTestPage.title"/>
            </h3>
            <div class="title">
                <fmt:message key="test.title"/>: ${sessionScope.startedTest.title}
            </div>
            <div class="description">
                <fmt:message key="test.description"/>: ${sessionScope.startedTest.description}
            </div>
            <div class="subject">
                <fmt:message key="test.subject"/>: ${sessionScope.startedTest.subject.name}
            </div>
            <div class="difficulty">
                <fmt:message key="test.difficulty"/>: ${sessionScope.startedTest.difficulty.name()}
            </div>
            <div class="username">
                <fmt:message key="student.resultTestPage.userName"/>: ${sessionScope.currentUser.name} ${sessionScope.currentUser.surname}
            </div>
            <div class="status">
                <fmt:message key="test.status"/>: ${sessionScope.testStatus.name()}
            </div>
            <div class="score">
                <fmt:message key="test.score"/>: ${sessionScope.testResult}%
            </div>
            <div class="buttons">
                <form action="${pageContext.request.contextPath}/student/resultPDF">
                    <button type="submit" formtarget="_blank">
                        ...
                        <fmt:message key="student.resultTestPage.createPDF"/>
                    </button>
                </form>
